        ...
        .run(move |event, elwt| {
            ...
            match event {
                winit::event::Event::AboutToWait => {
                    let events = state.resource_mut::<Events<WindowEvent>>();
                    events.clear();
                    ...
                    if window_id == app.window.platform().id() =>
                {
                    match event {
                        winit::event::WindowEvent::RedrawRequested => {
                            let globals = *state.resource::<GpuGlobals>();
                            let renderer = state.resource_mut::<Renderer>();
                        ...
                        winit::event::WindowEvent::KeyboardInput { event, .. } => {
                            if let PhysicalKey::Code(code) = event.physical_key {
                                let input = state.resource_mut::<Input>();
                                match event.state {
